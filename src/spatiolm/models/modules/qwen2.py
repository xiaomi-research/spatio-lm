...
class Qwen2V3RConfig(Qwen2Config):
    model_type = "qwen2_v3r"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        head_dim = kwargs.get("head_dim", hidden_size // num_attention_heads)
...
class Qwen2Vision3RLayer(nn.Module):
    def __init__(self, config: Qwen2V3RConfig, layer_idx: int):
        super().__init__()
        ...
        self.frame_layer = Qwen2DecoderLayer(config, layer_idx)
        self.frame_layer.self_attn.is_causal = False

        self.global_layer = Qwen2DecoderLayer(config, layer_idx)
...
class Qwen2Vision3RCondition(nn.Module):
    def __init__(self, config: Qwen2V3RConfig):
        super().__init__()
        ...
        self.layers = nn.ModuleList(
            [Qwen2Vision3RLayer(config, idx) for idx in self.layer_ids]
        )
...
# @auto_docstring
class Qwen2V3RModel(Qwen2PreTrainedModel):
    def __init__(self, config: Qwen2Config):
        ...
        self.vision_3r_cond = Qwen2Vision3RCondition(config)
...
# @auto_docstring
class Qwen2ForV3RCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = Qwen2V3RModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> model = Qwen2ForCausalLM.from_pretrained("Qwen/Qwen2-8B")
        >>> tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen2-8B")
...
__all__ = ["Qwen2V3RConfig", "Qwen2V3RModel", "Qwen2ForV3RCausalLM"]
